...
class Character(pygame.sprite.Sprite):
    ...
    def update_anim(self):
        ...
        if self.grow == True:
            # self.action = 0
            # print("width and height before grow: ",self.width,self.height)
            ...
            if self.big == True:
                ...
                #     pl.kill()
                pl = Character(x_pos-10, y_pos, "mario_big", 4)
                #     player_group.add(pl)
                # # self.rect = self.image.get_rect()
    ...
    def move(self,move_left,move_right):
        ...
        if self.rect.bottom + dy < 100 and self.alive == False:
            
            dy = 100 - self.rect.bottom
            self.vel_y = 0
            # self.on_land = False
            # if self.action == 2:
            #     self.action = 0        
        ...
        for point_brick in point_brick_group:
            ...
            if point_brick.rect.colliderect(self.rect.x + dx,self.rect.y,self.width,self.height):
                self.rect.x -=5 * self.direction             
            if point_brick.rect.colliderect(self.rect.x,self.rect.y + dy,self.width,self.height):  
                # if jumping up
                if self.vel_y < 0:
                    self.vel_y = 0
                    dy = point_brick.rect.bottom - self.rect.top
                    # empty_brick = Point_brick(point_brick.rect.x,point_brick.rect.y)
                    # empty_brick.update()
                    # empty_brick_group.add(empty_brick)
                    # point_brick.kill()                     
                    point_brick.image = pygame.image.load(f".\\images\\empty_brick.png")
                    
                    # if pnt_brick_active == 1:
                    #     # mushroom = Mushroom(point_brick.rect.x, point_brick.rect.top - 32)
        ...
        for mushroom_brick in mushroom_brick_group:
            # if pygame.sprite.spritecollide(self, point_brick_group, False):
            if mushroom_brick.rect.colliderect(self.rect.x + dx,self.rect.y,self.width,self.height):
                self.rect.x -=5 * self.direction           
            if mushroom_brick.rect.colliderect(self.rect.x,self.rect.y + dy,self.width,self.height):  
                # if jumping up
                if self.vel_y < 0:
                    self.vel_y = 0
        ...
        for flag in flag_group:
            ...
            if flag.rect.colliderect(self.rect.x + dx,self.rect.y,self.width,self.height):
                dx = 0
            # if flag.rect.colliderect(self.rect.x,self.rect.y + dy,self.width,self.height):
            #     # if jumping up
            #     if self.vel_y < 0:
            #         self.vel_y = 0
            #         dy = flag.rect.bottom - self.rect.top
            ...
            #         self.vel_y = 0
            #         self.on_land = False
            #         dy = flag.rect.top - self.rect.bottom                    
        ...
        for tile in world.obstacle_list:
            ...
            if tile[1].colliderect(self.rect.x,self.rect.y + dy,self.width,self.height) and self.alive:
                ...
                # falling down
                    ...
                    if self.grow == True:
                        dy = tile[1].top - self.rect.bottom
            # elif tile[1].colliderect(self.rect.x,self.rect.y + dy,self.width,self.height) and self.grow == True:
            #     # if jumping up
            #     if self.vel_y < 0:
            #         self.vel_y = 0
            #         dy = tile[1].bottom  - self.rect.top
            ...
            #         self.vel_y = 0
            #         self.on_land = False
            #         # dy = tile[1].top - (self.rect.y * 1.25)                 
        ...
        if self.name in ("mario","mario_big","mario_bullet"):
    ...
    def enemy_ai(self):
        ...
        # for enemy_flag in enemy_flag_group:
        #     # if pygame.sprite.spritecollide(self, brick_group, False):
        #     if enemy_flag.rect.colliderect(self.rect.x,self.rect.y + dy,self.width,self.height) and self.alive:
        #         enemy_flag.rect.y = self.rect.y
                # # if jumping up
                # if self.vel_y < 0:
                #     self.vel_y = 0
                #     dy = brick.rect.bottom - self.rect.top
        ...
        for tile in world.obstacle_list:
            ...
            if tile[1].colliderect(self.rect.x + dx,self.rect.y,self.width,self.height) and self.name == "goomba" and self.alive:
                ...
                # if self.name == "koopa" and tile[1].top + 2 < self.rect.bottom:
                #     if self.flip:
                #         self.flip = False
            ...
            for x, tile in enumerate(row):
                if tile >= 0:
                    ...
                    if tile == 2:
                        ...
                        k_enemy = Character(x*tile_size,y*tile_size,"koopa",2) # koopa
                        k_enemy_group.add(k_enemy)                                                
                    elif tile == 6:
                        pl = Character(x*tile_size,y*tile_size,"mario",4)
                        # player_group.add(pl) # player
                    elif tile == 7:
...
class Mushroom(pygame.sprite.Sprite):
    def __init__(self,x,y):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load(f".\\images\\mushroom\\mushroom.png")
        self.rect = self.image.get_rect()
        self.rect.center = (x,y)
        self.width = self.image.get_width()
        self.height = self.image.get_height()         
...
# update and draw groups
        ...
        if pygame.Rect.colliderect(pl.rect, g_enemy.rect):
            ...
            # player hit by goomba
                ...
                if pl.grow == True and g_enemy.rect.right >= pl.rect.left - 5:
                    # pl.action = 4
                    pl.grow = False
                    pl = Character(self.x, self.y, "mario", 4)
    ...
    for k_enemy in k_enemy_group:       
        ...
        # if pl.rect.x - k_enemy.rect.x == 100 or k_enemy.rect.x - pl.rect.x == 100:
        #     if k_enemy.alive == False:
        #         k_enemy.kill()                                        
